...
PLACE_LIST_CREATE_URL = reverse("places:place-list-create", kwargs={"version": "v1"})
PLACE_RUD = "places:place-rud"
...
class PlaceAPITest(APITestCase):
    ...
    def test_create(self):
        ...
        response = self.client.post(PLACE_LIST_CREATE_URL, data=data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data.get("name"), data["name"])

    def test_delete(self):
        """
        Ensure a admin user can delete a Place
        """
        self.client.force_authenticate(user=self.admin_user)
    ...
    def test_update_direction_urls_and_place(self):
        ...
        response = self.client.post(PLACE_LIST_CREATE_URL, data=place_with_directions)
